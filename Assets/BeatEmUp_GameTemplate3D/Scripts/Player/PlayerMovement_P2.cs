...
public class PlayerMovement_P2 : MonoBehaviour {
	...
	private UnitAnimator_P2 animator;
	...
	[Header("Stats")]
	public DIRECTION_P2 currentDirection;
    ...
    //--
    // LOOK UP if OnEnable automatically LOADS in Unity.
    //If so, this could be why we are getting the NULL error on Player 2 spawn when hitting P
	void OnEnable() {
		InputManager_P2.onCombatInputEvent += InputEventAction;
        InputManager_P2.onInputEvent += InputEvent;
	}
	...
	void Start(){
		...
		//find components
		if(!animator) animator = GetComponentInChildren<UnitAnimator_P2>();
	...
	void MoveAirborne(){
		if(!WallInFront() && PlayerInsideCamViewArea()) {
			...
			LookToDir((DIRECTION_P2)lastKnownDirection);
	...
	public DIRECTION_P2 getCurrentDirection() {
