...
public class PlayerCombat_P2 : MonoBehaviour, IDamagable<DamageObject> {

	[Header ("Linked Components")]
	public Transform weaponBone; //the bone were weapon will be parented on
	private UnitAnimator_P2 animator; //link to the animator component
	...
	[Header ("Stats")]
	public DIRECTION_P2 currentDirection; //the current direction
	...
	private DIRECTION_P2 defendDirection; //the direction while defending
	...
	[SerializeField]
	public bool targetHit; //true if the last hit has hit a target PRIVATE
	...
	private InputManager_P2 inputManager;
	private INPUTACTION_2 lastAttackInput;
	private DIRECTION_P2 lastAttackDirection;
	...
	void OnEnable(){
		InputManager_P2.onCombatInputEvent += CombatInputEvent;
		InputManager_P2.onInputEvent += MovementInputEvent;
	}
	...
	void MovementInputEvent(Vector2 inputVector){
		...
		if(Mathf.Abs(inputVector.x)>0) currentDirection = (DIRECTION_P2)dir;
	}
	...
	private void CombatInputEvent(INPUTACTION_2 action) {
		if (AttackStates.Contains (playerState.currentState) && !isDead) {
			...
			if (action == INPUTACTION_2.PUNCH && (playerState.currentState != UNITSTATE.PUNCH && NearbyEnemyDown()) && isGrounded) {
				if(GroundPunchData.animTrigger.Length > 0) doAttack(GroundPunchData, UNITSTATE.GROUNDPUNCH, INPUTACTION_2.PUNCH);
				return;
			}
			...
			if (action == INPUTACTION_2.PUNCH && playerState.currentState != UNITSTATE.PUNCH && playerState.currentState != UNITSTATE.KICK && isGrounded) {
				...
				if(PunchCombo[attackNum] != null && PunchCombo[attackNum].animTrigger.Length > 0) doAttack(PunchCombo[attackNum], UNITSTATE.PUNCH, INPUTACTION_2.PUNCH);
				return;
			}
			...
			if (action == INPUTACTION_2.KICK && (playerState.currentState != UNITSTATE.KICK && NearbyEnemyDown()) && isGrounded) {
				if(GroundKickData.animTrigger.Length > 0) doAttack(GroundKickData, UNITSTATE.GROUNDKICK, INPUTACTION_2.KICK);
				return;
			}
	...
	public void CheckForHit() {
		...
		//on weapon hit
		if(lastAttackInput == INPUTACTION_2.WEAPONATTACK && targetHit) currentWeapon.onHitSomething();
	}
	...
	public void Ready() {
		...
		if (continueKickCombo) {
			...
			if(KickCombo[attackNum] != null && KickCombo[attackNum].animTrigger.Length>0) doAttack(KickCombo[attackNum], UNITSTATE.KICK, INPUTACTION_2.KICK);
			return;
		}
	...
	public bool isFacingTarget(GameObject g) {
		return ((g.transform.position.x > transform.position.x && currentDirection == DIRECTION_P2.Left) || (g.transform.position.x < transform.position.x && currentDirection == DIRECTION_P2.Right));
	}
