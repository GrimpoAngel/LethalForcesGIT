...
public class CameraFollow : MonoBehaviour {
    ...
    public BoxCollider CurrentBackCollider; //LETHAL FORCES - restrict back movement from player
	...
	void Update () {
		if (target){
			...
			if (!UseWaveAreaCollider) {
                ...
                transform.position = new Vector3(Mathf.Clamp(currentX, CurrentAreaCollider.transform.position.x + AreaColliderViewOffset, CurrentBackCollider.transform.position.x + AreaColliderViewOffset), currentY, currentZ) + AdditionalOffset;
            }

            //LETHAL FORCES - Duplicate the SET CAM POSITION CODE FOR THE BACK COLLIDER
            if (CurrentBackCollider == null) UseWaveAreaCollider = false;
            if (!UseWaveAreaCollider)
            {
                transform.position = new Vector3(Mathf.Clamp(currentX, CurrentAreaCollider.transform.position.x + AreaColliderViewOffset, CurrentBackCollider.transform.position.x + AreaColliderViewOffset), currentY, currentZ) + AdditionalOffset;
            }
	...
	void SetPlayerAsTarget(){
		GameObject player = GameObject.FindGameObjectWithTag ("CamFollow");  //LETHAL FORCES - changed tag to Cam Follow object
		if (player != null) {
